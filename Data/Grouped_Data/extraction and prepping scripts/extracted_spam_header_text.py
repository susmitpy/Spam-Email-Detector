...
forwaded_line_pattern = re.compile(r"Forwarded message")
inline_image_pattern = re.compile(r"img +src",flags=re.IGNORECASE)

field_names = ["sender_email","reply_to_email","subject","forwaded","inline_image_count","Label"]
...
for file in os.listdir():
    ...
    try:
        ...
        forwaded_line = re.search(forwaded_line_pattern,txt)
        if forwaded_line:
            row["forwaded"] = 1
        else:
            row["forwaded"] = 0
